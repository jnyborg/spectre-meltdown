...
double get_accuracy(char* test, char* actual) {
    int correct = 0;
    int len = strlen(test) > strlen(actual) ? strlen(test) : strlen(actual);

    for (int i = 0; i < len; i++) {
        // If we're further than one of the strings length, they are trivially different.
        if (strlen(test) <= i || strlen(actual) <= i)
            continue;
        
        if (test[i] == actual[i])
            correct++;
    }
    return correct * 100.0 / (len * 100.0);
}
